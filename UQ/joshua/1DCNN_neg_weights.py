"""
1D CNN with HALT-style uncertainty features, weighted BCE loss, and Brier/F1 evaluation.
Filters parse failures, uses 25 features per token, and trains with class weighting.
"""
...
# ====================
# Training loop with weighted BCE, gradient clipping, and F1 logging
# ====================
def train_model(model, train_loader, val_loader, epochs=50, lr=1e-3, device='cuda',
    ...
    if neg_weight is not None:
        criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(neg_weight).to(device))
    else:
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y, lengths in train_loader:
            ...
            swapped_labels = 1 - batch_y
            loss = criterion(logits, swapped_labels)
            loss.backward()
...
def evaluate_model(model, loader, criterion, device='cuda'):
    ...
    with torch.no_grad():
        for batch_x, batch_y, lengths in loader:
            batch_x = batch_x.to(device)
            batch_y = batch_y.float().to(device)
            lengths = lengths.to(device)   # <-- FIX: move lengths to device
            mask = torch.arange(batch_x.size(1), device=device).unsqueeze(0) < lengths.unsqueeze(1)
            logits = model(batch_x, mask)
...
def main():
    ...
    neg_weight = (df['is_correct'] == 1).sum() / (df['is_correct'] == 0).sum()
    print(f"\nNegative class weight (N_pos/N_neg): {neg_weight:.4f}")
    ...
    try:
        model, history = train_model(model, train_loader, val_loader, epochs=50,
                                    lr=1e-3, device=device, neg_weight=neg_weight)
    ...
    print("\n" + "="*60)
    print("Test Set Evaluation (1D CNN with 25 features)")
    print("="*60)
