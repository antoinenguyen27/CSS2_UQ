...
from UQ.SE.config import ANSWER_LETTERS, K_CHOICES, MIN_VALID_SAMPLES
...
def semantic_entropy_from_answers(valid_answers: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "probabilities": {
            letter: float(probabilities_array[idx])
            for idx, letter in enumerate(ANSWER_LETTERS)
        },
        "entropy": entropy,
...
def build_results_frame(question_samples: list[dict[str, Any]], eval_split: str) -> pd.DataFrame:
    ...
    for question in question_samples:
        stats = semantic_entropy_from_answers(question["valid_answers"])
        ...
        rows.append(
            {
                ...
                "n_C": stats["counts"]["C"],
                "n_D": stats["counts"]["D"],
                "p_A": stats["probabilities"]["A"],
                "p_B": stats["probabilities"]["B"],
                "p_C": stats["probabilities"]["C"],
                "p_D": stats["probabilities"]["D"],
